                ...
                <div className="flex flex-wrap justify-center gap-8 md:gap-12">
                    {certifications.map((cert, index) => (
                        <motion.div
                            ...
                            className={`flex flex-col items-center group ${cert.link ? "cursor-pointer" : "cursor-default"}`}
                        >
                            {cert.link ? (
                                <a href={cert.link} target="_blank" rel="noopener noreferrer" className="flex flex-col items-center w-full focus:outline-none">
                                    <CertCard cert={cert} />
                                </a>
                            ) : (
    ...
    return (
        <>
            ...
            <div className={`w-36 h-36 md:w-44 md:h-44 rounded-full ${cert.bg} border ${cert.border} flex items-center justify-center p-6 relative overflow-hidden backdrop-blur-sm transition-all duration-500 shadow-[0_0_30px_rgba(0,0,0,0.3)] group-hover:shadow-[0_0_40px_rgba(255,255,255,0.1)]`}>
                ...
                {cert.image ? (
                    <div className="absolute inset-0 z-10 rounded-full overflow-hidden flex items-center justify-center bg-white">
                        <Image src={cert.image} alt={cert.title} fill className="object-cover scale-[1.02]" />
                    </div>
                ) : (
            ...
            <div className={`mt-6 text-center max-w-[180px] transition-colors ${cert.link ? "group-hover:text-white" : ""}`}>
                <p className="text-sm md:text-base font-medium text-neutral-200 leading-snug">
                    {cert.title}
                </p>
